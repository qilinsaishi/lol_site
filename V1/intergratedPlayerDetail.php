...
$data = [
    ...
    "hotPlayerList"=>["dataType"=>"intergratedPlayerList","game"=>$config['game'],'except_player'=>$pid,"page"=>1,"page_size"=>6,"fields"=>'pid,position,player_name,logo,team_id',"rand"=>1,"cacheWith"=>"currentPage","cache_time"=>86400*7],
...
<head>
  ...
  <meta name="viewport"
    ...
    <meta name="description" content="<?php echo strip_tags($return['intergratedPlayer']['data']['teamInfo']['team_name']);?><?php echo $return['intergratedPlayer']['data']['player_name'];?>，真名为<?php echo $return['intergratedPlayer']['data']['player_name'];?>，<?php echo $return['intergratedPlayer']['data']['country'];?>人，<?php if(!in_array($return['intergratedPlayer']['data']['position'],["","?"])){echo "在".$return['intergratedPlayer']['data']['teamInfo']['team_name']."中长期打".$return['intergratedPlayer']['data']['position'].".位置，";}?><?php if(count($return['intergratedPlayer']['data']['playerList'])>0){echo "与".implode(",",array_column($return['intergratedPlayer']['data']['playerList'],"player_name"))."为队友";}?>。">
